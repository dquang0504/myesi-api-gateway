...
async def risk_score(request: Request):
    ...
    try:
        ...
        if "sbom_id" not in params:
            raise HTTPException(status_code=400, detail="Missing sbom_id query parameter")

        logger.info(f"Forwarding /score request to Risk Service: {RISK_SERVICE_URL}/api/risk/score with params {params}")
        async with httpx.AsyncClient() as client:
            res = await client.get(f"{RISK_SERVICE_URL}/api/risk/score", params=params, timeout=30.0)
        return res.json()
    except Exception as e:
        logger.error(f"Error forwarding /score request: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Gateway -> Risk Service score error: {str(e)}"
        )
...
async def risk_trends(request: Request):
    ...
    try:
        params = dict(request.query_params)
        logger.info(f"Forwarding /trends request to Risk Service: {RISK_SERVICE_URL}/api/risk/trends with params {params}")
        async with httpx.AsyncClient() as client:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Gateway -> Risk Service trends error: {str(e)}"
        )